...
from panoptica.utils.input_check_and_conversion.check_sitk_image import sanity_checker_sitk_image
from panoptica.utils.input_check_and_conversion.check_torch_image import sanity_checker_torch_image
from panoptica.utils.input_check_and_conversion.check_nibabel_image import sanity_checker_nibabel_image
...
def post_check(prediction_reference_array_pair: tuple[np.ndarray, np.ndarray]) -> bool:
    ...
    """
    assert (
        isinstance(prediction_reference_array_pair, tuple) and len(prediction_reference_array_pair) == 2
    ...
    if not np.issubdtype(prediction_reference_array_pair[0].dtype, np.integer) or not np.issubdtype(
        prediction_reference_array_pair[1].dtype, np.integer
    ):
        warn(
    ...
    max_value = max(prediction_reference_array_pair[0].max(), prediction_reference_array_pair[1].max())
    dtype = _get_smallest_fitting_uint(max_value)
    prediction_reference_array_pair = (prediction_reference_array_pair[0].astype(dtype), prediction_reference_array_pair[1].astype(dtype))
