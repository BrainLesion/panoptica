...
def plot_box(
    ...
    if orientation == "v":
        ...
        for e in data.keys():
            fig.add_trace(go.Box(y=df_data.query(f'{graph_name} == "{e}"')[score], name=e, orientation=orientation))
    else:
        fig = px.strip(df_data, y=graph_name, x=score, stripmode="overlay", orientation=orientation)
        fig.update_traces(marker={"size": 5, "color": "#555555"})
        for e in data.keys():
            fig.add_trace(go.Box(x=df_data.query(f'{graph_name} == "{e}"')[score], name=e, orientation=orientation, boxpoints=False))
    fig.update_layout(
        autosize=False,
        width=width,
        height=height,
